"""
v5_manga_ocr_full_preprocess_test.py  (was: zelda_translator_manga_ocr_full_preprocess.py)
===========================================================================================
ITERATION 5 — A/B test: run Manga OCR with the same heavy preprocessing that
Apple Vision was receiving, to isolate whether preprocessing helps or hurts it.

What it does:
  Scenario A — full preprocessing applied before Manga OCR:
    1. White pixel isolation (threshold > 200 to extract dialogue text)
    2. Furigana row masking (mask rows where median blob height is small)
    3. 2× Lanczos upscale
    4. Output: binarized white-on-black image (identical to what Apple Vision got)
  Step 2 is unchanged: qwen2.5:7b via Ollama plain text translation.

Why this file exists:
  v4 tested Manga OCR with minimal preprocessing. This file tests the opposite
  extreme — maximum preprocessing — to answer: does Manga OCR prefer the raw
  colour crop (v4) or the binarized Apple-Vision-style crop (v5)?
  Result: Manga OCR is trained on natural manga images; binarization destroys
  the image statistics it expects and typically degrades accuracy vs v4.

Key differences from v4:
  - Full preprocessing pipeline re-enabled (identical to Apple Vision path)
  - Everything else identical: same bounds detection, same translation prompt,
    same brightness gate, same CSV logging

Install deps:
  pip install manga-ocr pillow

Run:  python3 v5_manga_ocr_full_preprocess_test.py
...
# Downloads ~400MB on first run, cached to ~/.cache/huggingface after that.
# Runs on CPU on M1 Pro — expect ~300-700ms per inference on the upscaled image.
...
def manga_ocr_run(frame):
    ...
    Receives the fully preprocessed white-on-black binarized+upscaled image
    — identical input to what Apple Vision was receiving.
    Converts in-memory to PIL Image (no temp file needed).
    """
    t0 = time.perf_counter()

    # BGR (OpenCV) → RGB (PIL) — Manga OCR expects PIL Image
...
def preprocess_crop(crop):
    """
    1. Extract pure white text pixels onto black background
    2. Black out sparse rows (furigana) — they have much lower pixel density
       than main text rows now that camera is closer and text is larger
    ...
    # Row density on the white-on-black mask
    ...
    # Black out rows below 20% of max density — these are furigana or gaps
...
def clean_ocr(text):
    """
    Strip non-Japanese characters from OCR output.
    Manga OCR can also emit stray symbols on high-contrast binarized input.
...
def translation_loop(cap, bounds):
    print("🌐  TRANSLATION — Manga OCR (full preprocessing) + qwen2.5:7b pipeline started.")

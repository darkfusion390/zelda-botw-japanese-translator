...
===================
Pipeline:
  Step 1 — OCR:         manga-ocr (Windows-compatible, transformer-based Japanese OCR)
...
# manga-ocr is a transformer model fine-tuned on stylised Japanese text (manga /
# game UI). It handles game fonts and furigana far better than generic OCR.
_tagger   = fugashi.Tagger()
_kakasi   = pykakasi.kakasi()
_jmd      = Jamdict()
_mocr     = MangaOcr()          # loads once; first call warms the model (~1-2 s)
...
def apple_vision_ocr(frame):
    """manga-ocr replacement for Apple Vision — Windows-compatible.

    manga-ocr is a transformer model fine-tuned on stylised Japanese text
    (manga / game UI fonts). It handles Zelda's game font and inline furigana
    significantly better than generic OCR engines.
    ...
    try:
        japanese = _mocr(Image.open(tmp_path))
    finally:
        os.unlink(tmp_path)

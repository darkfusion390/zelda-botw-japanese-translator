...
===================
Pipeline:
  Step 1 — OCR:         EasyOCR (Japanese, trained on diverse fonts including bold/italic)
...
# EasyOCR — trained on diverse text sources including printed/bold/italic fonts.
# More robust than manga-ocr on BotW's bold italic dialogue font.
# gpu=False for CPU-only; set True if CUDA available.
_tagger   = fugashi.Tagger()
_kakasi   = pykakasi.kakasi()
_jmd      = Jamdict()
_mocr     = easyocr.Reader(['ja'], gpu=False)  # loads once; downloads models on first run
...
def apple_vision_ocr(frame):
    """EasyOCR — replaces manga-ocr.

    EasyOCR is trained on diverse text sources and handles BotW's bold italic
    dialogue font more reliably than manga-ocr (which was trained on standard
    weight manga fonts and misreads characters like 火, 味, が in bold italic).

    EasyOCR returns a list of (bbox, text, confidence) tuples ordered
    top-to-bottom left-to-right. We join all detected text with spaces.
    ...
    try:
        results = _mocr.readtext(tmp_path, detail=1, paragraph=False)
    finally:
        os.unlink(tmp_path)

    # results is [(bbox, text, confidence), ...] — join all text in order
    parts = [text for (_, text, conf) in results if conf > 0.1 and text.strip()]
    japanese = " ".join(parts)
...
def preprocess_crop(crop):
    ...
    Steps:
      1. Greyscale.
      2. Denoise (fastNlMeans, h=7) — light denoise preserves thin stroke gaps
         (h=15 was merging 火's strokes into a blob, causing 火→水 misreads).
      3. Fixed threshold at 185 — isolates only bright white text strokes.
      4. Morphological closing REMOVED — was merging dakuten dots and fine stroke
         details (が→か, 味→咲 errors). Bold font needs strokes kept separate.
      5. Furigana row suppression.
      6. Deskew — BotW uses italic font (~8-14 degree slant). Shear transform
         restores upright strokes so manga-ocr sees characters it was trained on.
         SHEAR_FACTOR = 0.20 (≈ tan(11°), midpoint of measured 8-14° range).
         Tune up toward 0.25 if slanted chars still misread, down to 0.15 if
         upright chars start breaking.
      7. 2x upscale + padding.
      8. Invert — manga-ocr expects black-on-white.
    """
    gray = cv2.cvtColor(crop, cv2.COLOR_BGR2GRAY)

    # Step 2: denoise — flattens background texture to near-black
    ...
    # Step 4: morphological closing REMOVED — merges dakuten and fine strokes on
    # bold font causing が→か, 味→咲 errors. Strokes are clean enough without it.
    ...
    # Step 6: deskew — remove italic slant so manga-ocr sees upright strokes.
    # BotW dialogue uses ~8-14 degree italic. Shear left by SHEAR_FACTOR to
    # straighten. Canvas is widened to avoid clipping right edge of text.
    SHEAR_FACTOR = 0.20
    h, w = mask.shape[:2]
    M = np.float32([[1, SHEAR_FACTOR, 0], [0, 1, 0]])
    mask = cv2.warpAffine(mask, M, (w + int(h * SHEAR_FACTOR), h),
                          flags=cv2.INTER_LINEAR,
                          borderMode=cv2.BORDER_CONSTANT, borderValue=0)

    # Step 7: upscale + padding
    h, w = mask.shape[:2]
